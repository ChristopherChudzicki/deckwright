...
export type MagicItemIndexEntry = Omit<MagicItemDetail, "ruleset">;
...
export const fetchMagicItemIndex = async (ruleset: Ruleset): Promise<MagicItemIndex> => {
  ...
  const page = await apiGet<Open5ePage<MagicItemIndexEntry>>(path);
...
export const fetchMagicItemDetail = async (
  ...
  const raw = await apiGet<MagicItemIndexEntry>(`/v2/magicitems/${key}/`);
  return { ...raw, ruleset };
};
